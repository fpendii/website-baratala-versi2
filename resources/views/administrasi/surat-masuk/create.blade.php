...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Input Surat Masuk Baru</h5>
        </div>
        <div class="card-body">
            {{-- Pastikan enctype="multipart/form-data" ada untuk upload file --}}
            <form method="POST" action="{{ url('administrasi/surat-masuk/store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="pengirim" name="pengirim"
                            ...
                            placeholder="Nama atau Instansi Pengirim" value="{{ old('pengirim') }}" required>
                        @error('pengirim')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="nomor_surat" name="nomor_surat"
                            ...
                            placeholder="Contoh: 001/SK/III/2024" value="{{ old('nomor_surat') }}">
                        @error('nomor_surat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="prioritas" name="prioritas" class="form-select @error('prioritas') is-invalid @enderror"
                            required>
                            <option value="">-- Pilih Prioritas --</option>
                            <option value="rendah" {{ old('prioritas') == 'rendah' ? 'selected' : '' }}>Rendah</option>
                            <option value="sedang" {{ old('prioritas') == 'sedang' ? 'selected' : '' }}>Sedang</option>
                            <option value="tinggi" {{ old('prioritas') == 'tinggi' ? 'selected' : '' }}>Tinggi (Urgent)
                            </option>
                        </select>
                        @error('prioritas')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="keterangan" name="keterangan" class="form-control" rows="4"
                        placeholder="Tuliskan keterangan singkat mengenai isi surat atau tujuan." required>{{ old('keterangan') }}</textarea>
                </div>

                <button type="submit" class="btn btn-primary">
                    <i class="bx bx-save me-1"></i> Simpan Surat Masuk
                </button>
                <a href="{{ url('administrasi/surat-masuk') }}" class="btn btn-outline-secondary">Batal</a>
            </form>
        </div>
    </div>
@endsection
