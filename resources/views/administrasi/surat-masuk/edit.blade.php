...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ url('administrasi/surat-masuk/update', $suratMasuk->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    @if ($suratMasuk->lampiran)
                        <div class="alert alert-info d-flex justify-content-between align-items-center p-2 mb-2">
                            <span>File saat ini: <a href="{{ asset('storage/public/' . $suratMasuk->lampiran) }}" target="_blank" class="fw-bold">{{ basename($suratMasuk->lampiran) }}</a></span>
                        </div>
                    @endif
