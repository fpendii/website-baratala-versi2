                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="d-block text-muted">
                                <i class="bx bx-play-circle me-1"></i> Mulai: {{ \Carbon\Carbon::parse($item->tanggal_mulai)->format('d M Y') }}
                            </span>
                            {{-- Tanggal Selesai (Batas Waktu) --}}
                            <span class="d-block text-muted">
                                <i class="bx bx-calendar-check me-1"></i> Batas: {{ \Carbon\Carbon::parse($item->tanggal_selesai)->format('d M Y') }}
                            </span>
                        ...
                        <td class="text-center">
                            <div class="btn-group" role="group">
                                <a href="{{ route('karyawan.rencana.show', $item->id) }}" class="btn btn-icon btn-outline-info waves-effect" title="Lihat Detail">
                                    <i class="bx bx-show"></i>
                                </a>
                                <a href="{{ route('karyawan.rencana.edit', $item->id) }}" class="btn btn-icon btn-outline-warning waves-effect" title="Edit Rencana">
                                    <i class="bx bx-edit"></i>
                                </a>
                                <form action="{{ route('karyawan.rencana.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus rencana kerja: {{ $item->judul_rencana }}?')" class="d-inline">
                                    @csrf
                                    @method('DELETE')
