            ...
            <form action="{{ route('surat-keluar.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('tujuan') is-invalid @enderror"
                        name="tujuan" value="{{ old('tujuan') }}" placeholder="Contoh: Direktur Utama PT. Contoh Sejahtera" required>
                    @error('tujuan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('jenis_surat') is-invalid @enderror" name="jenis_surat" required>
                        ...
                        <option value="keuangan" {{ old('jenis_surat') == 'keuangan' ? 'selected' : '' }}>Keuangan</option>
                        <option value="operasional" {{ old('jenis_surat') == 'operasional' ? 'selected' : '' }}>Operasional</option>
                    </select>
                    @error('jenis_surat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                {{-- Perihal --}}
                <div class="mb-4">
                    <label class="form-label">Perihal (Isi Surat) <span class="text-danger">*</span></label>
                    <textarea class="form-control @error('perihal') is-invalid @enderror" rows="5" name="perihal"
                        placeholder="Masukkan ringkasan isi surat..." required>{{ old('perihal') }}</textarea>
                    @error('perihal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
