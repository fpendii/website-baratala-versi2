...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('direktur.rencana.create') }}" class="btn btn-primary btn-sm">Tambah Rencana Kerja</a>
</div>
        ...
        <table class="table table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->catatan ?? '-' }}</td>
                    <td>
                        <a href="{{ route('direktur.rencana.show', $item->id) }}" class="btn btn-sm btn-primary">Detail</a>
                        <a href="{{ route('direktur.rencana.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
                        <form action="{{ route('direktur.rencana.destroy', $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
